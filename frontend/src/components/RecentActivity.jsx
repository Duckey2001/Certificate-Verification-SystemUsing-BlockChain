  ...
  useEffect(() => {
    const fetchRealActivities = async () => {
      try {
        ...
        // Try to fetch from admin endpoint, but handle 404 gracefully
        let response;
        try {
          response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/api/admin/system-logs?limit=10`, {
            headers: {
          ...
          throw new Error('Network error fetching system logs');
        }
        
        // If endpoint doesn't exist (404), use fallback data
        if (response.status === 404) {
          console.log('Admin system logs endpoint not found, using fallback data');
          setActivities([
            {
              id: 1,
        ...
        if (!contentType || !contentType.includes('application/json')) {
          // If not JSON, try to get text and see what we received
