...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (window.google?.accounts?.id?.initialize) {
      console.warn('Google OAuth already initialized');
      return;
    }
    ...
    script.onload = () => {
      if (window.google && googleButtonRef.current && !window.google.accounts.id._isInitialized) {
    ...
    return () => {
      ...
      if (window.google?.accounts?.id) {
        window.google.accounts.id._isInitialized = false;
      }
    };
  }, []);
