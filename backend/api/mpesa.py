...
MPESA_CONSUMER_KEY = os.getenv("MPESA_CONSUMER_KEY", os.getenv("MPESA_LESOTHO_CONSUMER_KEY", ""))
MPESA_CONSUMER_SECRET = os.getenv("MPESA_CONSUMER_SECRET", os.getenv("MPESA_LESOTHO_CONSUMER_SECRET", ""))
MPESA_PASSKEY = os.getenv("MPESA_PASSKEY", os.getenv("MPESA_LESOTHO_PASSKEY", ""))
MPESA_SHORTCODE = os.getenv("MPESA_SHORTCODE", os.getenv("MPESA_LESOTHO_SHORTCODE", "110799"))
...
class MpesaSTKRequest(BaseModel):
    ...
    payment_type: Optional[str] = "phone"  # 'phone' or 'merchant'
...
class MpesaService:
    ...
    def initiate_stk_push(self, phone_number: str, amount: float, 
                         account_reference: str = "CertiVert-LS",
                         transaction_desc: str = "Certificate Verification Fee (Lesotho)",
                         payment_type: str = "phone") -> Dict[str, Any]:
        ...
        try:
            ...
            # Adjust transaction description based on payment type
            if payment_type == "merchant":
                transaction_desc = "Merchant CertiVert Fee"
            else:
...
async def initiate_stk_push(
    ...
    try:
        ...
        result = mpesa_service.initiate_stk_push(
            ...
            payment_type=request.payment_type or "phone"
        )
        ...
        return {
            ...
            "customer_message": result.get("CustomerMessage", "STK push sent. Please check your phone."),
            "response_description": result.get("ResponseDescription"),
            "status": "pending",
            "next_step": "Please check your phone for M-Pesa STK push prompt.",
            "instructions": [
                "1. You will receive a prompt on your M-Pesa number",
                "2. Enter your M-Pesa PIN to complete the payment",
                "3. Use CORRECT PIN to test successful payment",
                "4. Use INCORRECT PIN to test failure handling",
...
async def mpesa_callback(
    ...
    try:
        ...
        if result["success"]:
            ...
            # Log success
            print(f"✅ Payment successful: {payment.id} - Amount: {payment.mpesa_amount} LSL")
            ...
            if "insufficient funds" in result_desc.lower():
                error_message = "Payment failed: Insufficient funds in your M-Pesa account"
            elif "wrong pin" in result_desc.lower() or "incorrect pin" in result_desc.lower():
                error_message = "Payment failed: Incorrect M-Pesa PIN entered"
            elif "timeout" in result_desc.lower():
                error_message = "Payment failed: Transaction timed out - please try again"
            elif "cancelled" in result_desc.lower():
                error_message = "Payment cancelled by user"
            else:
                error_message = f"Payment failed: {result_desc}"
            
            payment.error_message = error_message
            print(f"❌ Payment failed: {payment.id} - {error_message}")
