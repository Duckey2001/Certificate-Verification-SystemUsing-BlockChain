...
@router.websocket("/verifier/{user_id}")
async def verifier_websocket(
    websocket: WebSocket,
    user_id: str,
    db: Session = Depends(get_db)
):
    """
    WebSocket endpoint for verifier-specific real-time updates
    """
    ...
    try:
        ...
        from models import User
        user = db.query(User).filter(User.id == user_id).first()
        if not user or user.role not in ["verifier", "admin"]:
        ...
        await websocket.send_json({
            ...
            "user_id": user_id,
            "message": f"Connected as verifier: {user.username}"
        })
        ...
        while True:
            try:
                ...
                print(f"WebSocket error for verifier {user_id}: {e}")
                break
                
    except WebSocketDisconnect:
        print(f"Verifier {user_id} disconnected")
    except Exception as e:
        print(f"WebSocket connection error for verifier {user_id}: {e}")
